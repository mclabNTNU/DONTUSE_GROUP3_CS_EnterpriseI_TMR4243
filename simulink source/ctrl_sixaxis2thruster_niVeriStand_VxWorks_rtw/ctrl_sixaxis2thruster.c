...
static void ctrl_sixaxis2thruster_output(void)
{
  ...
  /* '<S1>:1:5' */
  u1 = fabs(ctrl_sixaxis2thruster_B.PosXRight);
  if ((1.165 >= u1) || rtIsNaN(u1)) {
    u1 = 1.165;
  }

  if (ctrl_sixaxis2thruster_B.PosXRight < 0.0) {
    tmp = -1.0;
  } else if (ctrl_sixaxis2thruster_B.PosXRight > 0.0) {
    tmp = 1.0;
  } else if (ctrl_sixaxis2thruster_B.PosXRight == 0.0) {
    tmp = 0.0;
  } else {
    tmp = ctrl_sixaxis2thruster_B.PosXRight;
  }

  ctrl_sixaxis2thruster_B.u_VSP = u1 / 1.165 * tmp;

  /* '<S1>:1:6' */
  u1 = fabs(ctrl_sixaxis2thruster_B.PosYRight);
  if ((2.629 >= u1) || rtIsNaN(u1)) {
    u1 = 2.629;
  }

  if (ctrl_sixaxis2thruster_B.PosYRight < 0.0) {
    tmp = -1.0;
  } else if (ctrl_sixaxis2thruster_B.PosYRight > 0.0) {
    tmp = 1.0;
  } else if (ctrl_sixaxis2thruster_B.PosYRight == 0.0) {
    tmp = 0.0;
  } else {
    tmp = ctrl_sixaxis2thruster_B.PosYRight;
  }

  ctrl_sixaxis2thruster_B.u_BT = u1 * 0.3875 / 2.629 * tmp;

  /* '<S1>:1:7' */
  ctrl_sixaxis2thruster_B.alpha_VSP = 0.0;

  /* '<S1>:1:8' */
  ctrl_sixaxis2thruster_B.Omega_VSP = 0.3;
...
RT_MODEL_ctrl_sixaxis2thruste_T *ctrl_sixaxis2thruster(void)
{
  ...
  ctrl_sixaxis2thruster_M->Sizes.numBlocks = (12);/* Number of blocks */
  ctrl_sixaxis2thruster_M->Sizes.numBlockIO = (6);/* Number of block outputs */
  ctrl_sixaxis2thruster_M->Sizes.numBlockPrms = (55);/* Sum of parameter "widths" */
...
void SetExternalOutputs(double* data, int* TaskSampleHit)
{
  ...
  if (TaskSampleHit[0]) {              // sample and hold
    ni_extout[index++] = NIRT_GetValueByDataType
      (&ctrl_sixaxis2thruster_B.alpha_VSP,0,0,0);
  } else {
    index += 1;
  }
  ...
  if (TaskSampleHit[0]) {              // sample and hold
    ni_extout[index++] = NIRT_GetValueByDataType
      (&ctrl_sixaxis2thruster_B.Omega_VSP,0,0,0);
  } else {
    index += 1;
  }
...
{
  { 0, "ctrl_sixaxis2thruster/PosXRight", 0, "", offsetof
    (B_ctrl_sixaxis2thruster_T, PosXRight)+0*sizeof(real_T), BLOCKIO_SIG, 0, 1,
    2, 0, 0 },

  { 1, "ctrl_sixaxis2thruster/PosYRight", 0, "", offsetof
    (B_ctrl_sixaxis2thruster_T, PosYRight)+0*sizeof(real_T), BLOCKIO_SIG, 0, 1,
    2, 2, 0 },

  { 2, "ctrl_sixaxis2thruster/MATLAB Function", 0, "u_BT", offsetof
    (B_ctrl_sixaxis2thruster_T, u_BT)+0*sizeof(real_T), BLOCKIO_SIG, 0, 1, 2, 4,
    0 },

  { 3, "ctrl_sixaxis2thruster/MATLAB Function", 1, "u_VSP", offsetof
    (B_ctrl_sixaxis2thruster_T, u_VSP)+0*sizeof(real_T), BLOCKIO_SIG, 0, 1, 2, 6,
    0 },

  { 4, "ctrl_sixaxis2thruster/MATLAB Function", 2, "alpha_VSP", offsetof
    (B_ctrl_sixaxis2thruster_T, alpha_VSP)+0*sizeof(real_T), BLOCKIO_SIG, 0, 1,
    2, 8, 0 },

  { 5, "ctrl_sixaxis2thruster/MATLAB Function", 3, "Omega_VSP", offsetof
    (B_ctrl_sixaxis2thruster_T, Omega_VSP)+0*sizeof(real_T), BLOCKIO_SIG, 0, 1,
    2, 10, 0 },

  { -1, "", -1, "", 0, 0, 0 }
};

static int NI_SigListSize DataSection(".NIVS.siglistsize") = 6;
...
static char* NI_CompiledModelDateTime = "Thu Jan 26 15:23:41 2017";
